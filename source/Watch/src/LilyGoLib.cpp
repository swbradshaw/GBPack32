// taken from https://github.com/Xinyuan-LilyGO/TTGO_TWatch_Library/tree/t-watch-s3
...
bool LilyGoLib::begin(Stream *stream)
{
    ...
    if (!res) {
        ...
        setRemapAxes(REMAP_BOTTOM_LAYER_TOP_RIGHT_CORNER);
    ...
    if (!res) {
        ...
        setClockOutput(CLK_DISABLE);   //Disable clock output ， Conserve Backup Battery Current Consumption
    ...
    if (!res) {
        ...
        SensorDRV2605::setMode(0); // internal trigger mode
...
long LilyGoLib::getLastTouch()
{
    return lastTouch;
}
void LilyGoLib::setLastTouch(long lastTouch)
{
