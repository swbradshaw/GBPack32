...
class AfterlifeRing : public LedRing {
    public:

        AfterlifeRing(Adafruit_NeoPixel &strip, uint8_t startLED, uint8_t endLED, uint32_t color,  uint8_t visibleLEDS[], uint8_t visibleLEDCount) : LedRing(strip,startLED,endLED,color) {
            this->visibleLEDS = visibleLEDS;
            this->visibleLEDcount = visibleLEDCount;
        };
    ...
    private:
        ...
        const int16_t BOOT_TIME = 10000;
        const int16_t POWEROFF_TIME = 10000;
