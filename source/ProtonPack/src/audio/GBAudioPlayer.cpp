...
void GBAudioPlayer::sendEvent(EventArgs args) {
    Subject::notify(args);
}
...
void GBAudioPlayer::playFile(String filename) {
    playFile(filename, 0);
}
void GBAudioPlayer::setFollowEvent(String event) {
    postEvent = event;
}
