#include "menu.h"
#include <stdint.h>

/* ===================================================================== *
 *                                                                       *
 * Volume control                                                        *
 *                                                                       *
 * ===================================================================== *
 */


uint8_t g_volParam = 100; // when this value comes from an EEPROM, load it in setup
                          // at the moment here is no setup function (To-Do)

void correctAndSetVolume() {
    if (g_volParam > 100) {
        g_volParam = 100;
    }
    if (g_volParam < 0) {
        g_volParam = 0;
    }
    float volume = g_volParam / 100.0f;
    audioEngine.setVolume(volume);
}

void mVolume_para(uint8_t line)
// *********************************************************************
{
  if (LCDML.FUNC_setup()) // ****** SETUP *********
  {
    float volume = audioEngine.getVolume();
    // convert float to int volume
    g_volParam = volume * 100;
  }
  // check if this function is active (cursor stands on this line)
  if (line == LCDML.MENU_getCursorPos())
  {
    // make only an action when the cursor stands on this menu item
    //check Button
    if(LCDML.BT_checkAny())
    {
      if(LCDML.BT_checkEnter())
      {
        // this function checks returns the scroll disable status (0 = menu scrolling enabled, 1 = menu scrolling disabled)
        if(LCDML.MENU_getScrollDisableStatus() == 0)
        {
          // disable the menu scroll function to catch the cursor on this point
          // now it is possible to work with BT_checkUp and BT_checkDown in this function
          // this function can only be called in a menu, not in a menu function
          LCDML.MENU_disScroll();
        }
        else
        {
          // enable the normal menu scroll function
          LCDML.MENU_enScroll();
        }

        // do something
        // ...

        LCDML.BT_resetEnter();
      }

      // This check have only an effect when MENU_disScroll is set
      if(LCDML.BT_checkUp())
      {
        g_volParam+=10;
        correctAndSetVolume();
        LCDML.BT_resetUp();
      }

      // This check have only an effect when MENU_disScroll is set
      if(LCDML.BT_checkDown())
      {
        g_volParam-=10;
        correctAndSetVolume();
        LCDML.BT_resetDown();
      }


      if(LCDML.BT_checkLeft())
      {
        g_volParam+=10;
        correctAndSetVolume();
        LCDML.BT_resetLeft();
      }

      if(LCDML.BT_checkRight())
      {
        g_volParam-=10;
        correctAndSetVolume();
        LCDML.BT_resetRight();
      }
    }
  }

  char buf[20];
  sprintf (buf, "Volume: %d", g_volParam);

  // setup function
  u8g2.drawStr( getDispBoxX0() + getFontWidth() + getCursorSpaceBehind(),  (getFontHeight() * (1+line)), buf);     // the value can be changed with left or right

}